# EuropaKeplerOrbit.gd — elliptische Bahn RELATIV zu Jupiter
extends Node3D

@export var center_body: NodePath				# Jupiter (leer = Parent)
@export var AU_UNITS: float = 30.0				# 1 AU = 30 Units in deiner Szene
@export var distance_scale: float = 1.0			# zusätzl. Sichtbarkeits-Skala

# Bahndaten (J2000-nah, ekliptikbezogen)
@export var a_au: float = 0.004485				# ~670,900 km / AU
@export var e: float = 0.009					# Exzentrizität
@export var i_deg: float = 1.791				# Inklination zur Ekliptik
@export var Omega_deg: float = 0.0				# Länge aufst. Knoten (falls bekannt eintragen)
@export var omega_deg: float = 0.0				# Argument des Perizentrums (falls bekannt)

@export var days_per_orbit: float = 3.551181	# siderische Umlaufzeit (Tage)
...
func _ready() -> void:
	...
	if _center == null:
		_center = get_parent() as Node3D	# Standard: Parent ist Jupiter

func _process(delta: float) -> void:
	# KORREKT: simulierte Tage = delta * seconds_per_day * time_scale
	_t_days += delta / SimGlobals.seconds_per_day

	# Mittlere Anomalie
	var M := TAU * (_t_days / days_per_orbit) + deg_to_rad(mean_anomaly_deg_at_t0)

	# Kepler lösen -> exzentrische Anomalie E
	...
	# Position relativ zum Jupiter-Zentrum (Godot: Y=up)
