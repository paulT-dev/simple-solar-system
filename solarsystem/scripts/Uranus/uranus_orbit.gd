# UranusKeplerOrbit.gd — elliptische Bahn um (0,0,0)
extends Node3D

@export var AU_UNITS: float = 30.0			# 1 AU = 30 Units
@export var a_au: float = 19.191			# große Halbachse (AU)
@export var e: float = 0.0472				# Exzentrizität
@export var i_deg: float = 0.770			# Inklination (°)
@export var Omega_deg: float = 74.23		# Länge aufst. Knoten (°)
@export var omega_deg: float = 96.74		# Argument des Perihels (°)

@export var days_per_orbit: float = 30685.0	# ~84 Jahre in Tagen
@export var mean_anomaly_deg_at_t0: float = 0.0	# Startphase (M0 in °)

var t_days := 0.0

func _process(delta: float) -> void:
	# simulierte Tage (global skaliert) aufsummieren
