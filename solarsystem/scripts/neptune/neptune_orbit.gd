# NeptuneKeplerOrbit.gd — elliptische Bahn um (0,0,0)
extends Node3D

@export var AU_UNITS: float = 30.0			# 1 AU = 30 Units
@export var a_au: float = 30.07				# große Halbachse (AU)
@export var e: float = 0.008678			# Exzentrizität
@export var i_deg: float = 1.769			# Inklination (°)
@export var Omega_deg: float = 131.784		# Länge aufst. Knoten (°)
@export var omega_deg: float = 272.846		# Argument des Perihels (°)

@export var days_per_orbit: float = 60190.0	# ~164.8 Jahre in Tagen
@export var mean_anomaly_deg_at_t0: float = 0.0

var t_days := 0.0

func _process(delta: float) -> void:
	# simulierte Tage aufsummieren (global)
